// Build script:
//   1. Bundle src/code.js → dist/code.js  (esbuild, includes helpers.js)
//   2. Compile Tailwind  → dist/styles.css
//   3. Inline CSS + JS   → dist/ui.html   (what Figma loads)
...
// ── 2 + 3. Inline CSS and JS into dist/ui.html ───────────────────────────────
...
const js    = fs.readFileSync(path.join(root, 'src', 'ui.js'), 'utf-8');
const code  = fs.readFileSync(path.join(root, 'dist', 'code.js'), 'utf-8');
// Self-contained @font-face rules (base64 woff2). Inlined so the plugin loads
// no external resources, complying with Figma's CSP. Regenerate with
// scripts/fonts/build-fonts.js.
const fonts = fs.readFileSync(path.join(root, 'scripts', 'fonts', 'fonts.inline.css'), 'utf-8');

if (!html.includes('/* INLINE_FONTS */')) {
  console.error('ERROR: placeholder /* INLINE_FONTS */ not found in src/ui.html');
  process.exit(1);
}
...
if (!html.includes('/* INLINE_JS */')) {
  console.error('ERROR: placeholder /* INLINE_JS */ not found in src/ui.html');
  process.exit(1);
}

// ── 4. Generate cache buster hash ─────────────────────────────────────────────

const hashInput = code + css + js + fonts;
